...
#ifndef	__COMMON_READERS__
#define	__COMMON_READERS__

#include	"virtual-reader.h"

class	reader_16: public virtualReader {
public:
	reader_16	(RingBuffer<std::complex<float>> *p, int32_t);
	~reader_16	(void);
...
public:
	reader_24	(RingBuffer<std::complex<float>> *p, int32_t);
	~reader_24	(void);
void	processData	(float IQoffs, void *data, int cnt);
int16_t bitDepth	(void);
};
...
public:
	reader_32	(RingBuffer<std::complex<float>> *p, int32_t);
	~reader_32	(void);
void	processData	(float IQoffs, void *data, int cnt);
int16_t	bitDepth	(void);
};
...
public:
	reader_float	(RingBuffer<std::complex<float>> *p);
	~reader_float	(void);
void	processData	(float IQoffs, void *data, int cnt);
int16_t	bitDepth	(void);
};
