...
class TimeSync {
public:
	TimeSync (const float iSps, const float iAlpha):
	                                       mSmplPerSym(iSps),
	                                       mAlpha(iAlpha) {}
...
//	   const DSPFLOAT mm_val = y - x;
	   ...
	   const DSPFLOAT x = real (mSampBuffRail [2] -
	                            mSampBuffRail [0]) * real(mSmplBuff[1])  +
	                      imag (mSampBuffRail [2] -
	                            mSampBuffRail[0]) * imag(mSmplBuff[1]);
	   const DSPFLOAT y = real (mSmplBuff [2] -
	                            mSmplBuff [0]) * real(mSampBuffRail [1]) +
	                      imag (mSmplBuff [2] - mSmplBuff [0]) *
	                            imag(mSampBuffRail[1]);

	   const DSPFLOAT mm_val = y - x;

	   mMu	+= mSmplPerSym + mAlpha * mm_val;
//	round down to nearest int since we are using it as an index:
	   mSkipNoSmpl = (int32_t)(/*round*/(mMu));
